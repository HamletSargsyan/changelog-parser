...
def load(fp: IOBase, encoding: str = "utf-8") -> list[dict[str, Any]]:
    ...
    while line := fp.readline():
        ...
        if line.startswith("## ") and not in_compare_urls:
            ...
            line = line.removeprefix("### ")
            if line.lower() not in CHANGE_TYPES:
                raise ChangelogParsingError(
                    f'Invalid change type, "{ line }"', line_no, 5
                )
            if line.lower() in changes[-1]:
                raise ChangelogParsingError(
                    f'Multiple "{ line }" sections found', line_no, 5
                )
            ...
            if section is None:
                raise ChangelogParsingError(
                    "Change not under a category section", line_no
                )
            ...
            for change in changes:
                if isinstance(change["version"], str) and isinstance(version, str):
                    if change["version"] == version:
                        break
                if isinstance(change["version"], Version) and isinstance(
                    version, Version
                ):
                    if change["version"] == version:
                        break
            ...
            raise ChangelogParsingError(
                f'Unrecognized line pattern, "{ line }"', line_no
            )
...
def dump(
    ...
    def encode(i):
        return i if isinstance(fp, TextIOBase) else i.encode(encoding)
